...
class keyword_copy extends StatefulWidget {
  @override
  State<keyword_copy> createState() => _keyword_copyState();
}

class _keyword_copyState extends State<keyword_copy> {
  ...
  List<Performance> performances =
      []; // Performance(단어 저장) 리스트 : ListTile 위젯 리스트(UI)를 추가하기 위한 변수
  ...
  @override
  void dispose() {
    // 텍스트에디팅컨트롤러를 제거, 등록된 리스너 제거하는 메소드
    txtWord.dispose();
    super.dispose();
  }
  ...
  Future saveUserWord() async {
    // 사용자가 입력한 단어를 저장하는 메소드 (AlertDialog의 save 버튼 누를 시 호출됨)
  ...
  List<Widget> getContent() {
    ...
    performances.forEach((performance) {
      ...
      tiles.add(Dismissible(
          // 삭제를 위해 Dismissible 위젯 추가
          key: UniqueKey(), // 위젯을 고유하게 식별 가능하도록 함
          onDismissed: (_) {
            // 스크롤 방향을 받는 매개변수 입력 가능하지만 사용하지 않은 상태
            helper
